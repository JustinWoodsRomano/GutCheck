...
// The bulk build uses CUTOFF to keep the citywide payload manageable. A
// single restaurant page has no such constraint and people reasonably
// expect to see a place's whole record, so its scoped fetch reaches back
// to the start of the city's published data.
export const HISTORY_CUTOFF = "2010-01-01T00:00:00.000";

// Genuine sit-down bars/taverns, as an explicit, hand-curated allow-list --
// verified directly against the live feed's actual distinct facility_type
// values, not guessed or wildcard-matched. Chicago's dataset records these
// as wildly inconsistent free text; a naive "%BAR%"/"%LIQUOR%" match would
// also sweep in JUICE BAR, PROTEIN SHAKE BAR, MASSAGE BAR (not
// alcohol-serving venues) and plain retail liquor stores (LIQUOR STORE,
// GROCERY/LIQUOR) that nobody sits down and drinks at. Deliberately
// conservative -- ambiguous entries like "1475 LIQUOR" and "RETAIL
// WINE/WINE BAR" (retail-primary naming) are left out.
export const BAR_FACILITY_TYPES = [
  "BAR", "BAR/GRILL", "TAVERN", "TAVERN/LIQUOR", "TAVERN-LIQUOR",
  "TAVERN/1006", "TAVERN/PACKAGED GOODS", "TAVERN/RESTAURANT",
  "TAP ROOM/TAVERN/LIQUOR STORE", "TAVERN/BAR", "RESTAURANT/BAR",
  "RESTAURANT/BAR/THEATER", "RESTUARANT AND BAR", "RESTAURANT AND LIQUOR",
  "LIQUOR CONSUMPTION ON PREMISES.", "LIQUORE STORE/BAR",
  "SERVICE BAR/THEATRE", "SHUFFLEBOARD CLUB WITH BAR", "HOOKA BAR",
  "WINE TASTING BAR", "THEATER/BAR",
];

// Shared SoQL facility_type clause for both the full build-time pipeline
// and per-restaurant ISR revalidation, so a bar can never be present in
// one dataset and silently absent from the other.
export function facilityTypeWhereClause() {
  const list = BAR_FACILITY_TYPES.map((t) => `'${t.replace(/'/g, "''")}'`).join(",");
  return `(facility_type='Restaurant' OR upper(facility_type) IN (${list}))`;
}
...
export function parseViolations(raw) {
  ...
  return raw
    ...
    .map((chunk) => {
      ...
      const comment = (parts[1] || chunk).replace(/\s+/g, " ").trim(); // no length cap -- was truncating real comments mid-sentence
      const isCritical = /PRIORITY FOUNDATION|PRIORITY VIOLATION/.test(chunk);
      // Keep the city's own violation code and title. The comment alone is
      // the inspector's free text, which can't be aggregated -- the numbered
      // code is the city's actual classification and is what makes
      // "most-cited violation" computable.
      const header = (parts[0] || "").trim();
      const m = header.match(/^(\d{1,2})\.\s*(.+)$/);
      return {
        t: comment,
        s: isCritical ? "c" : "n",
        ...(m ? { c: Number(m[1]), ct: m[2].replace(/\s+/g, " ").trim() } : {}),
      };
    });
}
...
export function mergeVisits(entries) {
  ...
  return order.map((d) => {
    ...
    // Keep why the visit happened. Complaint-driven inspections fail at a
    // much higher rate than routine ones, so the reason is real signal --
    // not just metadata.
    return {
      date: d,
      grade,
      ...
      violations,
      sawOutOfBusiness,
      inspectionType: gradeRow?.inspection_type || rows[0]?.inspection_type || null,
    };
  });
}
...
export function buildRestaurantFromRows(entries, { neighborhoodFor, historyLimit = 5 }) {
  ...
  const gradedVisits = visits.filter((v) => v.grade !== null);
  const history = (
    Number.isFinite(historyLimit) ? gradedVisits.slice(0, historyLimit) : gradedVisits
  ).map((v) => ({ d: v.date, g: v.grade, v: v.violations, it: v.inspectionType }));
...
export async function fetchRowsForRestaurant(rawName, rawAddress, license) {
  ...
  // Addresses in the city feed are inconsistently padded -- the same
  // storefront appears as both '3042 N BROADWAY' and '3042 N BROADWAY '.
  // An exact match splits one restaurant's record in two and silently
  // drops the older half, which is usually the half under a previous
  // license. trim() on both sides stitches them back together.
  const addressClause = `(${nameClause} AND upper(trim(address))=upper(trim('${esc(rawAddress)}')))`;
  ...
  const params = new URLSearchParams({
    ...
    $where: `${facilityTypeWhereClause()} AND inspection_date >= '${HISTORY_CUTOFF}' AND (${addressClause}${licenseClause})`,
    $order: "inspection_date DESC",
    // 16 years of rows for one restaurant. The busiest records in the
    // dataset run to a few dozen inspections, and rows are per-violation
    // before mergeVisits() collapses them, so 500 leaves real headroom.
    $limit: "500",
  });
