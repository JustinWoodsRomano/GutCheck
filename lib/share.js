...
import { GRADE_EMOJI } from "./constants";
...
// framing and no editorializing about the restaurant. Leads with the same
// grade emoji used everywhere else on the site (GRADE_EMOJI), so a shared
// message still visually signals FAIL/CONDITIONAL at a glance.
export function getAnonymizedViolationShareMessage(violation, grade) {
  const label = violation?.s === "c" ? "priority violation" : "violation";
  const emoji = GRADE_EMOJI[grade] || "\uD83D\uDEA8";
  return `${emoji} A Chicago restaurant was cited for a ${label} at its most recent health inspection: "${violation.t}" \u2014 GutCheck Chicago, official public health inspection records.`;
}
